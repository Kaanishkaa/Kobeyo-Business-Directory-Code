...
class Business:
    ...
    job_categories: List[str]
    skill_ids: List[int]
    skill_names: List[str]  # NEW FIELD
...
class BusinessSkillTagger:
    def __init__(self, openai_api_key: str, skill_rules_csv: str):
        ...
        # Load tagging rules with skill names
        self.rules_df = pd.read_csv(skill_rules_csv)
        self.rules_df = self.rules_df[
            self.rules_df["Prompt Rule"].notna() & self.rules_df["Skills IDs"].notna()
    ...
    def tag_business_with_skills(self, text: str) -> (List[int], List[str]):
        matched_ids = set()
        matched_names = set()

        for _, row in self.rules_df.iterrows():
            keywords = re.split(r",|\n|;", str(row["Prompt Rule"]).lower())
            if any(kw.strip() and kw.strip() in text.lower() for kw in keywords):
                # Extract skill IDs
                ids = [
                    int(i.strip())
                ...
                # Extract skill names
                names = [
                    s.strip() for s in str(row["Skills Names"]).split(",") if s.strip()
                ]
                matched_names.update(names)

        return sorted(matched_ids), sorted(matched_names)
    ...
    def process_business(
        ...
        analysis = self.analyze_business_with_ai(name, types, reviews, summary)
        description = analysis.get("description", "")
        skill_ids, skill_names = self.tag_business_with_skills(description)
