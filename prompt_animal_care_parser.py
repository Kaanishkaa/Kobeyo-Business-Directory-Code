    ...
    def search_all_places(self, query: str, location: str) -> List[Dict]:
        ...
        while True:
            ...
            if not next_page_token:
                break
            # wait before next page token becomes active
            import time
...
#     places = tagger.search_places(query, location)[:10]
...
if __name__ == "__main__":
    ...
    # ✅ Step 1: Load existing entries (Name + Address) from CSV
    existing_keys = set()
    try:
        ...
        if "Name" in existing_df.columns and "Address" in existing_df.columns:
            ...
            print("CSV does not have 'Address' column. Skipping deduplication.")
