...
MODEL = "gpt-3.5-turbo"  # fast & inexpensive; swap to "gpt-4o" if desired
MAX_SITE_CHARS = 3000  # clip long pages to save tokens + cost
###############################################################################

# Load your OpenAI key from env var
openai.api_key = ""
...
def classify_catering(page_text: str) -> bool:
    """
    Ask OpenAI whether the business is a caterer.  Returns True/False.
    """
    ...
    messages = [
        {
            ...
            "content": (
                ...
                "Answer the user's question with only the word YES or NO."
            ),
        },
        {
            "role": "user",
            "content": (
                "Does this company provide food *catering* services "
                "(event/wedding/banquet, corporate catering, meal-prep catering, etc.)?\n\n"
                f'WEBSITE TEXT:\n"""\n{page_text}\n"""'
            ),
        },
    ]
